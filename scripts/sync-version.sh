...
INIT_PY="$PROJECT_ROOT/src/confluence_assistant_skills/__init__.py"
...
get_version_from_file() {
    ...
    case "$file" in
        ...
        *pyproject.toml)
            ...
            ;;
        *__init__.py)
            # Check if using dynamic versioning from importlib.metadata
            if grep -q 'from importlib.metadata import version' "$file" 2>/dev/null; then
                echo "dynamic"
            else
    ...
    case "$file" in
        ...
        *plugin.json|*marketplace.json)
            if command -v jq &> /dev/null; then
                ...
                # Also update nested plugin version in marketplace.json
                if [[ "$file" == *"marketplace.json" ]]; then
                    tmp=$(mktemp)
                    jq ".plugins[0].version = \"$new_version\"" "$file" > "$tmp" && mv "$tmp" "$file"
                fi
            else
...
check_versions() {
    ...
    local files=("$PLUGIN_JSON" "$MARKETPLACE_JSON" "$PYPROJECT" "$INIT_PY")
    ...
    for file in "${files[@]}"; do
        if [[ -f "$file" ]]; then
            ...
            if [[ "$file_version" == "dynamic" ]]; then
                success "$relative_file: (dynamic from importlib.metadata)"
...
main() {
    case "${1:-sync}" in
        ...
        --help|-h)
            ...
            echo "  - pyproject.toml"
            echo "  - src/confluence_assistant_skills/__init__.py"
            ;;
        *)
