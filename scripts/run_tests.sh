...
#   ./scripts/run_tests.sh --skill confluence-page  # Run specific skill only
...
echo ""
echo -e "${BLUE}══════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Unit Tests - Confluence Assistant Skills${NC}"
if $COVERAGE; then
...
for skill in "${SKILLS[@]}"; do
    ...
    case $status in
        PASS)
            printf "%-25s %10s %10s ${GREEN}%10s${NC}\n" "$skill" "$passed" "$failed" "PASS"
            ;;
        FAIL)
            printf "%-25s %10s %10s ${RED}%10s${NC}\n" "$skill" "$passed" "$failed" "FAIL"
            ;;
        SKIP)
