...
Examples:
  ...
  python list_properties.py 12345 --output json
    ...
    parser.add_argument('--sort', choices=['key', 'version'], default='key',
                        help='Sort properties by field (default: key)')
    parser.add_argument('--expand', help='Comma-separated fields to expand (e.g., version)')
        ...
        if not properties:
            ...
            for i, prop in enumerate(properties, 1):
                ...
                if args.verbose:
                    ...
                    # Compact output
                    value_str = str(value) if not isinstance(value, dict) else format_json(value)
                    if len(value_str) > 60:
