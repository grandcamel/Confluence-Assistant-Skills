...
Note: Exactly one of --resolve or --unresolve must be specified.
    ...
    action_group.add_argument(
        "--resolve", "-r", action="store_true", help="Mark comment as resolved"
    )
    action_group.add_argument(
    ...
    current_comment = client.get(
        f"/api/v2/footer-comments/{comment_id}", operation="get current comment"
    )

    # Prepare update with resolution status
    comment_data = {
        "version": {"number": current_comment["version"]["number"] + 1},
        "body": current_comment.get("body", {"representation": "storage", "value": ""}),
