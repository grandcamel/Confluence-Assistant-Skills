...
def create_remote_link_data(
    page_id: str, issue_key: str, jira_url: str, relationship: str = "relates to"
) -> dict[str, Any]:
    """
    ...
    return {
        ...
        "application": {"type": "jira", "name": "JIRA"},
        "relationship": relationship,
        "object": {
            "url": jira_url,
            "title": issue_key,
            "icon": {"url16x16": "/images/icons/jira_16.png"},
        },
    }
...
def add_jira_link_to_content(content: str, issue_key: str, jira_url: str) -> str:
    """
...
Examples:
  ...
  python link_to_jira.py 12345 PROJ-123 --jira-url https://jira.example.com
    ...
    parser.add_argument(
        "--jira-url", help="Base JIRA URL (e.g., https://jira.example.com)"
    )
    parser.add_argument(
    ...
    existing_links_response = client.get(
        f"/rest/api/content/{page_id}/remotelink", operation="get remote links"
    )
    ...
    if link_exists(existing_links, issue_key):
        if args.skip_if_exists:
            if args.output == "json":
                print(
                    format_json({"status": "skipped", "reason": "Link already exists"})
                )
            else:
