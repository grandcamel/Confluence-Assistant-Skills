...
Examples:
  ...
  python get_space_content.py DOCS --limit 100
    ...
    parser.add_argument(
        "--depth",
        choices=["root", "children", "all"],
        default="all",
        help="Content depth (default: all)",
    )
    parser.add_argument(
        "--status", choices=["current", "archived", "draft"], help="Filter by status"
    )
    parser.add_argument(
        "--include-archived", action="store_true", help="Include archived content"
    )
    parser.add_argument(
    ...
    # Build params
    params = {"space-id": space_id, "limit": min(limit, 25)}
    ...
    pages = list(
        client.paginate(
            "/api/v2/pages", params=params, limit=limit, operation="get space content"
        )
    )
    ...
    if args.output == "json":
        print(
            format_json({"space_key": space_key, "pages": pages, "count": len(pages)})
        )
    else:
        if not pages:
