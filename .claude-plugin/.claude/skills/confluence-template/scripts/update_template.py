...
Examples:
  ...
  python update_template.py tmpl-123 --remove-labels "old-tag"
    ...
    parser.add_argument('--file', help='File with new template content (Markdown or HTML)')
    ...
    if not any([args.name, args.description, args.content, args.file,
                args.add_labels, args.remove_labels]):
    ...
    current = client.get(
        ...
        operation='get current template'
    )
