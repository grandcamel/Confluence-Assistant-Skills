...
Examples:
  ...
  python list_templates.py --output json
    ...
    parser.add_argument(
        "--type", choices=["page", "blogpost"], help="Filter by template type"
    )
    parser.add_argument(
        ...
        if not results:
            ...
            for item in results:
                if args.blueprints:
                    # Blueprint format
                    bp_id = item.get(
                        "blueprintId", item.get("contentBlueprintId", "N/A")
                    )
    ...
    print_success(
        f"Listed {len(results)} {'blueprints' if args.blueprints else 'templates'}"
    )
