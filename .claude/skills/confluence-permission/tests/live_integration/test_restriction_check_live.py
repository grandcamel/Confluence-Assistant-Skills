...
Usage:
    pytest test_restriction_check_live.py --profile development -v
"""
...
class TestRestrictionCheckLive:
    ...
    def test_page_without_restrictions(self, confluence_client, test_page):
        ...
        for r in results:
            user_restrictions = r.get('restrictions', {}).get('user', {}).get('results', [])
            group_restrictions = r.get('restrictions', {}).get('group', {}).get('results', [])
