...
from formatters import print_success, format_attachment, format_json, format_table
...
Examples:
  ...
  python list_attachments.py 123456 --media-type application/pdf
    ...
    parser.add_argument('--limit', '-l', type=int, help='Maximum number of attachments to return')
    parser.add_argument('--media-type', '-m', help='Filter by media type (e.g., application/pdf, image/png)')
    ...
    if args.output == 'json':
        ...
        if not attachments:
            ...
            print(format_table(
                table_data,
                columns=['ID', 'Title', 'Type', 'Size', 'Version']
            ))
    else:
        # Text format
        if not attachments:
